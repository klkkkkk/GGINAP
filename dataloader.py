...
class MyDataset(Dataset):
    def __init__(self, configs, fold_id, data_type, data_dir=DATA_DIR):
        ...
        self.bert_token_lens_list, self.graph_list, self.emo_pos_list, self.cau_pos_list, \
    ...
    def __getitem__(self, idx):
        ...
        bert_token_idx, bert_clause_idx, bert_clause_sep_idx = self.bert_token_idx_list[idx], self.bert_clause_idx_list[
            idx], \
        ...
        return doc_couples, y_emotions, y_causes, doc_len, doc_id, bert_token_idx, bert_segments_idx, bert_clause_idx, \
               bert_clause_sep_idx, bert_token_lens, graph, emo_pos, cau_pos, y_emotion_category
    ...
    def read_data_file(self, data_type):
        ...
        for doc in data_list:
            ...
            graph, emo_pos, cau_pos = construct_graph(doc_len, self.K, memory)
        ...
        return doc_couples_list, y_emotions_list, y_causes_list, doc_len_list, doc_id_list, bert_token_idx_list, \
               bert_clause_idx_list, bert_clause_sep_idx_list, bert_segments_idx_list, bert_token_lens_list, \
               graph_list, emo_pos_list, cau_pos_list, y_emotion_category_list
...
def construct_graph(seq_len, k, memory):
    ...
    for i in range(seq_len):
        d[('emo', 'EE', 'emo')].append((i, i))
        d[('cau', 'CC', 'cau')].append((i, i))
        for j in range(i + 1, seq_len):
            d[('emo', 'EE', 'emo')].append((i, j))
            d[('emo', 'EE', 'emo')].append((j, i))
            d[('cau', 'CC', 'cau')].append((i, j))
            d[('cau', 'CC', 'cau')].append((j, i))
    ...
    if seq_len > k + 1:
        rel_mask = np.array(list(map(lambda x: -k <= x <= k, rel_pos.tolist())), dtype=np.int64)
    ...
    d = construct_pair_graph(d, seq_len, emo_pos, cau_pos)

    for emo, cau in zip(emo_pos.tolist(), cau_pos.tolist()):
        d[('emo', 'EP', 'pair')].append((emo, count))
        d[('cau', 'CP', 'pair')].append((cau, count))
        d[('pair', 'PE', 'emo')].append((count, emo))
        d[('pair', 'PC', 'cau')].append((count, cau))
        count += 1

    graph = dgl.heterograph(d)
    memory[seq_len] = graph, emo_pos, cau_pos
    return graph, emo_pos, cau_pos


def construct_pair_graph(d, doc_len, emo_pos, cau_pos):
    d[('pair', 'PEP', 'pair')] = []
    d[('pair', 'PCP', 'pair')] = []
    indices = torch.arange(len(emo_pos)).to(DEVICE)
    for x in range(doc_len):
        emo_mask = emo_pos == x
        cau_mask = cau_pos == x
        emo_indices = indices.masked_select(emo_mask)
        cau_indices = indices.masked_select(cau_mask)
        d[('pair', 'PEP', 'pair')].extend(construct_full_graph(emo_indices))
        d[('pair', 'PCP', 'pair')].extend(construct_full_graph(cau_indices))
    return d
...
def bert_batch_preprocessing(batch):
    ...
    bert_clause_sep_b, bert_token_lens_b, graphs, emo_pos, cau_pos, y_emotion_category_b = zip(*batch)
    ...
    for i in range(bert_clause_b.size(0)):
        ...
        for j in range(bert_clause_b.size(1)):
            if bert_clause_sep_b[i][j] == 0:
                break
            bert_clause_len.append(int(bert_clause_sep_b[i][j] - bert_clause_b[i][j] - 1))
        bert_clause_len_b.append(bert_clause_len)

    bsz, max_len = bert_token_b.size()
    bert_masks_b = np.zeros([bsz, max_len], dtype=np.float64)
    ...
    return np.array(doc_len_b), np.array(adj_b), np.array(y_emotions_b), np.array(y_causes_b), np.array(y_mask_b), \
           ...
           bert_clause_len_b, list(graphs), list(emo_pos), list(cau_pos), list(y_emotion_category_b)
